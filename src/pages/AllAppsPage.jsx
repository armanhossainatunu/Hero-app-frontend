...
const handelSelect = (e) => {
  ...
  return (
    <div className="mt-16">
      ...
      <div className="btn-group flex justify-center gap-4 mb-10">
        {currentPage > 0 && <button onClick={()=>setCurrentPage(currentPage-1)} className="btn">Previous</button>}
        {[...Array(totalpage).keys()].map((i) => (
          <button onClick={()=>setCurrentPage(i)} className={`btn ${i === currentPage ? "btn-primary" : ""}`}>{i}</button>
        ))}
        {currentPage + 1 < totalpage && <button onClick={()=>setCurrentPage(currentPage+1)} className="btn">Next</button>}
      </div>
    </div>
  );
};
